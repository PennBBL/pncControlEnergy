...
for i = 1:length(rho)
    ...
    ResultantFolder = [FA_ResultantFolder '/InitialAll0_TargetMeanActivationZScore_rho_' rho_Str];;
    EnergyCal_SGE_Function(Lausanne125_FA_Matrix_Cell, T, xc, x0, xf, rho(i), Type, ResultantFolder);
end
...
for i = 1:length(rho)
  rho_Str = strrep(num2str(rho(i)), '.', '');
  EnergyMat_Path = [FA_ResultantFolder '/FA_InitialAll0_TargetMeanActivationZScore_rho_' rho_Str '.mat'];
...
for i = 1:length(rho)
  ...
  save([FA_ResultantFolder '/FA_InitialAll0_TargetMeanActivationZScore_rho_' rho_Str '.mat'], 'Distance', 'Distance_sum', '-append');
end
