...
# Import modularity Q #
#######################
Modularity_Q_Mat <- readMat(paste0(ReplicationFolder, '/data/Modularity_Q/Modularity_Yeo_Q_FA.mat'));
...
Gam_WholeBrainAvg <- gam(Energy_WholeBrainAvg ~ s(Age_years, k=4) + Modularity_Q + Sex_factor + HandednessV2 + MotionMeanRelRMS + TBV + Strength_EigNorm_SubIden, method = "REML", data = Behavior);
...
for (i in 1:SystemsQuantity)
{
  ...
  Energy_Gam <- gam(tmp_variable ~ s(Age_years, k=4) + nbackBehAllDprime + Modularity_Q_Cognition + Sex_factor + HandednessV2 + MotionMeanRelRMS + TBV + Strength_EigNorm_SubIden, method = "REML", data = Behavior_New);
...
Energy_Gam_Cognition_CSV <- file.path(ResultantFolder, 'Energy_Gam_Cognition_YeoSystemLevel_RegressModularityQ.csv');
write.csv(Energy_Gam_Cognition_CSV, Energy_Gam_Cognition_CSV);
Energy_Gam_Cognition_Mat <- file.path(ResultantFolder, 'Energy_Gam_Cognition_YeoSystemLevel_RegressModularityQ.mat');
