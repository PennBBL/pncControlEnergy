...
ModControl_WholeBrainAvg = rowMeans(ModControl);
...
Gam_WholeBrainAvg <- gam(Energy_WholeBrainAvg ~ s(Age_years, k=4) + ModControl_WholeBrainAvg + Sex_factor + HandednessV2 + MotionMeanRelRMS + TBV + Strength_EigNorm_SubIden, method = "REML", data = Behavior);
...
for (i in 1:SystemsQuantity)
{
  ...
  Energy_Gam <- gam(tmp_variable ~ s(Age_years, k=4) + nbackBehAllDprime + ModControl_Yeo_Cognition[,i] + Sex_factor + HandednessV2 + MotionMeanRelRMS + TBV + Strength_EigNorm_SubIden, method = "REML", data = Behavior_New);
...
Energy_Gam_Cognition_CSV <- file.path(ResultantFolder, 'Energy_Gam_Cognition_YeoSystemLevel_RegressSystemModControl.csv');
write.csv(Energy_Gam_Cognition_CSV, Energy_Gam_Cognition_CSV);
Energy_Gam_Cognition_Mat <- file.path(ResultantFolder, 'Energy_Gam_Cognition_YeoSystemLevel_RegressSystemModControl.mat');
