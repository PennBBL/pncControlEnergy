...
function EnergyCal_ScaleEig_IndividualTarget_SGE_Function(ConnPathCell, scale_factor, T, xc, x0, xf_Cell, rho, Type, ResultantFolder)
...
for i = 1:length(ConnPathCell)
  ...
  pipeline.(Job_Name).command = 'EnergyCal_ScaleEig_Function(opt.para1, opt.para2, opt.para3, opt.para4, opt.para5, opt.para6, opt.para7, opt.para8)';
  ...
  pipeline.(Job_Name).opt.para6 = xf_Cell{i};
