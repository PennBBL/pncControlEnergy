...
#
  ...
  for (i in 1:length(Times_IDRange)) 
  {
    ...
    if (!file.exists(file.path(ResultantFolder_I, 'Res.mat'))) 
    {
      ...
      R_cmd = paste('source("/data/jux/BBL/projects/pncControlEnergy/scripts/Energy_Activation_Relation/InitialDM1_TargetActivation/ZC_CCA.R");
                     library(R.matlab);
  ...
  for (i in 1:Submit_Quantity) 
  {
    ...
    #system(cmd); 
    system(paste('sh ', ResultantFolder_I, '/script.sh', sep = ''));
  }
...
CCA_Permutation_SGE_Sub_function <- function(BrainMatrixPath, BehaviorMatrixPath, ResultantFolder) 
{
  ...
  Behavior_Dim <- dim(BehaviorMatrix);
  Subjects_Quantity <- Behavior_Dim[1];
  Behavior_Quantity <- Behavior_Dim[2];
  Rand_Index <- sample(1:Subjects_Quantity, Subjects_Quantity);
  BehaviorMatrix_Random <- matrix(c(1:(Subjects_Quantity * Behavior_Quantity)), nrow = Subjects_Quantity, ncol = Behavior_Quantity);  
  BehaviorMatrix_Random[1:Subjects_Quantity,] <- BehaviorMatrix[Rand_Index,];

  CCA_function(BrainMatrix, BehaviorMatrix_Random, file.path(ResultantFolder, 'Res.mat'));
}
