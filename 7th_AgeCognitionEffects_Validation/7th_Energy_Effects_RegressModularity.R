...
Energy_Gam <- gam(Energy_WholeBrainAvg ~ s(Age_years, k=4) + Modularity_Q + Sex_factor + HandednessV2 + MotionMeanRelRMS + TBV + Strength_EigNorm_SubIden, method = "REML", data = Behavior);
...
Energy_Gam_Cognition_CSV <- file.path(ResultantFolder, 'Energy_Gam_nbackBehAllDprime_YeoSystemLevel_RegressModularityQ.csv');
write.csv(Energy_Gam_Cognition_CSV, Energy_Gam_Cognition_CSV);
Energy_Gam_Cognition_Mat <- file.path(ResultantFolder, 'Energy_Gam_nbackBehAllDprime_YeoSystemLevel_RegressModularityQ.mat');
