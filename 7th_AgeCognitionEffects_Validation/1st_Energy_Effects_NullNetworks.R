...
ResultantFolder <- paste0(ReplicationFolder, '/results/InitialAll0_TargetMeanActivation_NullNetworks');
...
for (i in c(1:100))
{
  Energy_Mat_Path <- paste0(Energy_Data_Folder, '/NullNetworks/InitialAll0_TargetActivationMean_NullNetwork_', as.character(i), '.mat');
  ...
  for (j in 1:8)
  {
    System_I_Index <- intersect(which(Yeo_7Systems == j), SmallerThanNull_Index);
    Energy_YeoAvg[, j] <- rowMeans(Energy[, System_I_Index]);
  }        
  ...
  Energy_Gam_Cognition_CSV <- file.path(ResultantFolder, paste0('Energy_Gam_nbackBehAllDprime_YeoSystemLevel_NullNetwork_', as.character(i), '.csv'));
  write.csv(Energy_Gam_Cognition_CSV, Energy_Gam_Cognition_CSV);
  Energy_Gam_Cognition_Mat <- file.path(ResultantFolder, paste0('Energy_Gam_nbackBehAllDprime_YeoSystemLevel_NullNetwork_', as.character(i), '.mat'));
  ...
  Energy_Gam_Cognition <- matrix(0, nrow = 154, ncol = 3, dimnames = list(RowName_Nodal, ColName));
  for (j in 1:154)
  {
    tmp_variable <- Energy_Cognition[, j];
    Energy_Gam <- gam(tmp_variable ~ s(Age_years, k=4) + nbackBehAllDprime + Sex_factor + HandednessV2 + MotionMeanRelRMS + TBV + Strength_EigNorm_SubIden_Cognition, method = "REML", data = Behavior_New);
    Energy_Gam_Cognition[j, 1] <- summary(Energy_Gam)$p.t[2];
    Energy_Gam_Cognition[j, 2] <- summary(Energy_Gam)$p.pv[2];
  }
  Energy_Gam_Cognition[, 3] <- p.adjust(Energy_Gam_Cognition[, 2], "fdr");
  # Storing the results in both .csv and .mat file
  Energy_Gam_Cognition_CSV <- file.path(ResultantFolder, paste0('Energy_Gam_nbackBehAllDprime_NodalLevel_NullNetwork_', as.character(i), '.csv'));
  write.csv(Energy_Gam_Cognition, Energy_Gam_Cognition_CSV);
  Energy_Gam_Cognition_Mat <- file.path(ResultantFolder, paste0('Energy_Gam_nbackBehAllDprime_NodalLevel_NullNetwork_', as.character(i), '.mat'));
  ...
  # Accuracy score of executive function
  ...
  Behavior_New$TBV <- Behavior$TBV[NonNANIndex];
  Behavior_New$Strength_EigNorm_SubIden <- Behavior$Strength_EigNorm_SubIden[NonNANIndex];
  Behavior_New$F1_Exec_Comp_Res_Accuracy <- F1_Exec_Comp_Res_Accuracy[NonNANIndex];
  Strength_EigNorm_SubIden_Cognition <- Strength_EigNorm_SubIden[NonNANIndex];
  Energy_Cognition <- Energy[NonNANIndex,];
  Energy_YeoAvg_Cognition <- Energy_YeoAvg[NonNANIndex,];
  ...
  Energy_Gam_Cognition_YeoAvg[, 3] <- p.adjust(Energy_Gam_Cognition_YeoAvg[, 2], "fdr");
  print(Energy_Gam_Cognition_YeoAvg);
  Energy_Gam_Cognition_CSV <- file.path(ResultantFolder, paste0('Energy_Gam_F1ExecCompResAccuracy_YeoSystemLevel_NullNetwork_', as.character(i), '.csv'));
  write.csv(Energy_Gam_Cognition_CSV, Energy_Gam_Cognition_CSV);
  Energy_Gam_Cognition_Mat <- file.path(ResultantFolder, paste0('Energy_Gam_F1ExecCompResAccuracy_YeoSystemLevel_NullNetwork_', as.character(i), '.mat'));
  writeMat(Energy_Gam_Cognition_Mat, Cognition_Z = Energy_Gam_Cognition_YeoAvg[, 1], Cognition_P = Energy_Gam_Cognition_YeoAvg[, 2], Cognition_P_FDR = Energy_Gam_Cognition_YeoAvg[, 3]);
  print('###### Cognition effect of energy at nodal level ######');
  Energy_Gam_Cognition <- matrix(0, nrow = 154, ncol = 3, dimnames = list(RowName_Nodal, ColName));
  for (j in 1:154)
  ...
  Energy_Gam_Cognition[, 3] <- p.adjust(Energy_Gam_Cognition[, 2], "fdr");
  # Storing the results in both .csv and .mat file
  Energy_Gam_Cognition_CSV <- file.path(ResultantFolder, paste0('Energy_Gam_F1ExecCompResAccuracy_NodalLevel_NullNetwork_', as.character(i), '.csv'));
  write.csv(Energy_Gam_Cognition, Energy_Gam_Cognition_CSV);
  Energy_Gam_Cognition_Mat <- file.path(ResultantFolder, paste0('Energy_Gam_F1ExecCompResAccuracy_NodalLevel_NullNetwork_', as.character(i), '.mat'));
  writeMat(Energy_Gam_Cognition_Mat, Cognition_Z = Energy_Gam_Cognition[, 1], Cognition_P = Energy_Gam_Cognition[, 2], Cognition_P_FDR = Energy_Gam_Cognition[, 3]);
}
