...
for (i in c(1:100))
{
  ...
  Energy_Gam_Cognition_CSV <- file.path(ResultantFolder, paste0('Energy_Gam_nbackBehAllDprime_YeoSystemLevel_ShuffledTarget_', as.character(i), '.mat'));
  write.csv(Energy_Gam_Cognition_CSV, Energy_Gam_Cognition_CSV);
  Energy_Gam_Cognition_Mat <- file.path(ResultantFolder, paste0('Energy_Gam_nbackBehAllDprime_YeoSystemLevel_ShuffledTarget_', as.character(i), '.mat'));
  ...
  # Storing the results in both .csv and .mat file
  Energy_Gam_Cognition_CSV <- file.path(ResultantFolder, paste0('Energy_Gam_nbackBehAllDprime_NodalLevel_ShuffledTarget_', as.character(i), '.csv'));
  write.csv(Energy_Gam_Cognition, Energy_Gam_Cognition_CSV);
  Energy_Gam_Cognition_Mat <- file.path(ResultantFolder, paste0('Energy_Gam_nbackBehAllDprime_NodalLevel_ShuffledTarget_', as.character(i), '.mat'));
  ...
  Energy_Gam_Cognition_YeoAvg[, 3] <- p.adjust(Energy_Gam_Cognition_YeoAvg[, 2], "fdr");
  print(Energy_Gam_Cognition_YeoAvg); 
  Energy_Gam_Cognition_CSV <- file.path(ResultantFolder, paste0('Energy_Gam_F1ExecCompResAccuracy_YeoSystemLevel_ShuffledTarget_', as.character(i), '.csv'));
  write.csv(Energy_Gam_Cognition_CSV, Energy_Gam_Cognition_CSV);
  Energy_Gam_Cognition_Mat <- file.path(ResultantFolder, paste0('Energy_Gam_F1ExecCompResAccuracy_YeoSystemLevel_ShuffledTarget_', as.character(i), '.mat'));
  writeMat(Energy_Gam_Cognition_Mat, Cognition_Z = Energy_Gam_Cognition_YeoAvg[, 1], Cognition_P = Energy_Gam_Cognition_YeoAvg[, 2], Cognition_P_FDR = Energy_Gam_Cognition_YeoAvg[, 3]);
  print('###### Cognition effect of energy at nodal level ######');
  Energy_Gam_Cognition <- matrix(0, nrow = 154, ncol = 3, dimnames = list(RowName_Nodal, ColName));
  for (j in 1:154)
  ...
  Energy_Gam_Cognition[, 3] <- p.adjust(Energy_Gam_Cognition[, 2], "fdr");
  # Storing the results in both .csv and .mat file
  Energy_Gam_Cognition_CSV <- file.path(ResultantFolder, paste0('Energy_Gam_F1ExecCompResAccuracy_NodalLevel_ShuffledTarget_', as.character(i), '.csv'));
  write.csv(Energy_Gam_Cognition, Energy_Gam_Cognition_CSV);
  Energy_Gam_Cognition_Mat <- file.path(ResultantFolder, paste0('Energy_Gam_F1ExecCompResAccuracy_NodalLevel_ShuffledTarget_', as.character(i), '.mat'));
  writeMat(Energy_Gam_Cognition_Mat, Cognition_Z = Energy_Gam_Cognition[, 1], Cognition_P = Energy_Gam_Cognition[, 2], Cognition_P_FDR = Energy_Gam_Cognition[, 3]);
}
