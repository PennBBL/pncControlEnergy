...
Behavior_New$Sex <- Behavior$Sex[NANIndex];
...
ResultantFolder <- '/data/joy/BBL/projects/pncControlEnergy/results/ExecFun_Prediction/gamFeatureSelection';

# Split training samples and test samples
TrainingIndex <- c(1:500);
TestingIndex <- c(501:947);
Energy_training <- Energy[TrainingIndex,];
Behavior_New_training <- data.frame(Age_years = Behavior_New$Age_years[TrainingIndex]);
...
writeMat(file.path(ResultantFolder, 'trainData.mat'), Energy_training = Energy_training, Age_years = Behavior_New_training$Age_years, Sex = as.numeric(Behavior_New_training$Sex), HandednessV2 = as.numeric(Behavior_New_training$HandednessV2), MotionMeanRelRMS = Behavior_New_training$MotionMeanRelRMS, TBV = Behavior_New_training$TBV, OverallAccuracy = Behavior_New_training$OverallAccuracy);

Energy_testing <- Energy[TestingIndex,];
Age_years <- Behavior_New$Age_years[TestingIndex];
Sex <- as.numeric(Behavior_New$Sex[TestingIndex]);
HandednessV2 <- as.numeric(Behavior_New$HandednessV2[TestingIndex]);
...
writeMat(file.path(ResultantFolder, 'testData.mat'), Energy_testing = Energy_testing, Age_years = Age_years, Sex = Sex, HandednessV2 = HandednessV2, MotionMeanRelRMS = MotionMeanRelRMS, TBV = TBV, OverallAccuracy = OverallAccuracy);
...
for (i in 1:RegionsQuantity)
{
  ...
  Energy_Gam <- gam(Energy_training[,i] ~ s(Age_years, k=4) + OverallAccuracy + Sex_factor + HandednessV2 + MotionMeanRelRMS + TBV, method = "REML", data = Behavior_New_training);
