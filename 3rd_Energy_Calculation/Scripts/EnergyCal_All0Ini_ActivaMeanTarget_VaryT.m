...
for i = 1:length(T)
  ...
  ResultantFolder = [EnergyFolder '/InitialAll0_TargetActivationMean_T_' T_Str];
  EnergyCal_SGE_Function(Lausanne125_FA_Matrix_Cell, T(i), xc, x0, xf, S, rho, ResultantFolder);
end
...
for i = 1:length(T)
  T_Str = strrep(num2str(T(i)), '.', '');
  EnergyMat_Path = [EnergyFolder '/InitialAll0_TargetActivationMean_T_' T_Str '.mat'];
