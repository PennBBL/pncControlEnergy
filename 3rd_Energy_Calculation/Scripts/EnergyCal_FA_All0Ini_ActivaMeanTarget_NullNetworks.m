...
for i = 36:100
  Lausanne125_FA_Matrix_Cell = g_ls([Matrix_Folder '/FA_NullNetworks/NullNetwork_' num2str(i, '%03d') '/*.mat']);
  ResultantFolder = [FA_ResultantFolder '/InitialAll0_TargetActivationMean_NullNetwork_' num2str(i)];
...
for i = 1:100
  ...
  EnergyMat_Path = [FA_ResultantFolder '/FA_InitialAll0_TargetActivationMean_NullNetwork_' num2str(i) '.mat'];
  ...
  for j = 1:233
    Energy_New(:, j) = tmp.Energy(:, j) / abs(xf(j));
  end
  for j = 1:8
