...
Energy_Gam <- gam(Energy_WholeBrain ~ s(AgeYears, k=4) + Modularity_Q + Sex_factor + HandednessV2 + MotionMeanRelRMS + TBV + Strength_EigNorm_SubIden, method = "REML", data = Behavior);
...
{ 
  ...
  Energy_Gam <- gam(tmp_variable ~ s(AgeYears, k=4) + Modularity_Q + Sex_factor + HandednessV2 + MotionMeanRelRMS + TBV + Strength_EigNorm_SubIden, method = "REML", data = Behavior);
  ...
  Energy_lm <- lm(tmp_variable ~ AgeYears + Modularity_Q + Sex_factor + HandednessV2 + MotionMeanRelRMS + TBV + Strength_EigNorm_SubIden, data = Behavior);
