...
for (i in c(1:100))
{
  Energy_Mat_Path <- paste0(Energy_Data_Folder, '/ShuffledTargets/InitialAll0_TargetFPShuffle_', as.character(i), '.mat');
...
for (i in c(1:100))
{
  ...
  # Storing the results in both .csv and .mat file
  Energy_Gam_Cognition_CSV <- file.path(ResultantFolder, paste0('Energy_Gam_Cognition_NodalLevel_ShuffledTarget_', as.character(i), '.csv'));
  write.csv(Energy_Gam_Cognition_New, Energy_Gam_Cognition_CSV);
  Energy_Gam_Cognition_Mat <- file.path(ResultantFolder, paste0('Energy_Gam_Cognition_NodalLevel_ShuffledTarget_', as.character(i), '.mat'));
