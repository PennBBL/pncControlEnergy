...
# L1_ratio_Range = np.linspace(0.2, 1, 10);
# Parallel_Quantity = 3;
# Max_Queued = 256;
...
def ElasticNet_3Fold(Training_Data_Fold12, Training_Score_Fold12, Testing_Data_Fold3, Testing_Score_Fold3, \
    ...
    Predict_Corr_Fold12_training_Fold3_testing, Predict_MAE_Fold12_training_Fold3_testing = ElasticNet_APredictB(Training_Data_Fold12, \
        Training_Score_Fold12, Testing_Data_Fold3, Testing_Score_Fold3, Alpha_Range, L1_ratio_Range, 3, ResultantFolder_Sub1, Parallel_Quantity, Permutation_Flag, 0, 0)
    Predict_Corr_Fold13_training_Fold2_testing, Predict_MAE_Fold13_training_Fold2_testing = ElasticNet_APredictB(Training_Data_Fold13, \
        Training_Score_Fold13, Testing_Data_Fold2, Testing_Score_Fold2, Alpha_Range, L1_ratio_Range, 3, ResultantFolder_Sub2, Parallel_Quantity, Permutation_Flag, 0, 0)
    Predict_Corr_Fold23_training_Fold1_testing, Predict_MAE_Fold23_training_Fold1_testing = ElasticNet_APredictB(Training_Data_Fold23, \
        Training_Score_Fold23, Testing_Data_Fold1, Testing_Score_Fold1, Alpha_Range, L1_ratio_Range, 3, ResultantFolder_Sub3, Parallel_Quantity, Permutation_Flag, 0, 0)
...
def ElasticNet_APredictB_Permutation(Training_Data, Training_Score, Testing_Data, Testing_Score, Times_IDRange, \
                PriorParameters_Flag, AlphaRange_Or_PriorAlpha, L1_ratio_Range_Or_PriorL1ratio, Rand_Index_FilePath, ResultantFolder, Parallel_Quantity, Max_Queued, QueueOptions):
    if not os.path.exists(ResultantFolder):
        os.mkdir(ResultantFolder)
    ...
    for i in np.arange(len(Times_IDRange)):
        ...
        if not os.path.exists(ResultantFolder_I + '/APredictB.mat'):
            Times_IDRange_Todo = np.insert(Times_IDRange_Todo, len(Times_IDRange_Todo), Times_IDRange[i])
            Configuration_Mat = {'Subjects_Data_Mat_Path': Subjects_Data_Mat_Path, 'Subjects_Score_Mat_Path': Subjects_Score_Mat_Path, \
                'PriorParameters_Flag':PriorParameters_Flag, 'AlphaRange_Or_PriorAlpha': AlphaRange_Or_PriorAlpha, 'L1_ratio_Range_Or_PriorL1ratio': L1_ratio_Range_Or_PriorL1ratio, \
                'Rand_Index_FilePath': Rand_Index_FilePath, 'ResultantFolder_I': ResultantFolder_I, 'Parallel_Quantity': Parallel_Quantity, 'Times_I': Times_IDRange[i]};
            sio.savemat(ResultantFolder_I + '/Configuration.mat', Configuration_Mat)
            system_cmd = 'python3 -c ' + '\'import sys;\
                sys.path.append("/lustre/gaolab/cuizaixu/Utilities_Zaixu/Utilities_Regression/ElasticNet");\
                ...
                Subjects_Data_Mat_Path = configuration["Subjects_Data_Mat_Path"];\
                Subjects_Score_Mat_Path = configuration["Subjects_Score_Mat_Path"];\
                PriorParameters_Flag = configuration["PriorParameters_Flag"];\
                AlphaRange_Or_PriorAlpha = configuration["AlphaRange_Or_PriorAlpha"];\
                L1_ratio_Range_Or_PriorL1ratio = configuration["L1_ratio_Range_Or_PriorL1ratio"];\
                Rand_Index_FilePath = configuration["Rand_Index_FilePath"];\
                ResultantFolder_I = configuration["ResultantFolder_I"];\
                Parallel_Quantity = configuration["Parallel_Quantity"];\
                Times_I = configuration["Times_I"];\
                ElasticNet_APredictB_Permutation_Sub(Subjects_Data_Mat_Path[0], Subjects_Score_Mat_Path[0], PriorParameters_Flag[0][0], AlphaRange_Or_PriorAlpha[0], L1_ratio_Range_Or_PriorL1ratio[0], ResultantFolder_I[0], Parallel_Quantity[0][0], Rand_Index_FilePath[0], Times_I[0][0])\' ';
            system_cmd = system_cmd + ' > "' + ResultantFolder_I + '/perm_' + str(Times_IDRange[i]) + '.log" 2>&1\n'
...
def ElasticNet_APredictB_Permutation_Sub(Subjects_Data_Mat_Path, Subjects_Score_Mat_Path, PriorParameters_Flag, AlphaRange_Or_PriorAlpha, L1_ratio_Range_Or_PriorL1ratio, ResultantFolder, Parallel_Quantity, Rand_Index_FilePath, Times_I):
    # Data
    data = sio.loadmat(Subjects_Data_Mat_Path)
    ...
    # Parameters
    if PriorParameters_Flag:
        Prior_Alpha = AlphaRange_Or_PriorAlpha[Times_I]
        Prior_L1_ratio = L1_ratio_Range_Or_PriorL1ratio[Times_I]
        data = sio.loadmat(Rand_Index_FilePath)
        Rand_Index_All = data['Rand_Index']
        Prior_Rand_Index = Rand_Index_All[Times_I, :]
        ElasticNet_APredictB(Training_Data, Training_Score, Testing_Data, Testing_Score, Prior_Alpha, Prior_L1_ratio, 3, ResultantFolder, Parallel_Quantity, 1, 1, Prior_Rand_Index)
    else:
        ElasticNet_APredictB(Training_Data, Training_Score, Testing_Data, Testing_Score, AlphaRange_Or_PriorAlpha, L1_ratio_Range_Or_PriorL1ratio, Nested_Fold_Quantity, ResultantFolder, Parallel_Quantity, 1, 0)
                            
def ElasticNet_APredictB(Training_Data, Training_Score, Testing_Data, Testing_Score, AlphaRange_Or_PriorAlpha, L1_ratio_Range_Or_PriorL1ratio, Nested_Fold_Quantity, ResultantFolder, Parallel_Quantity, Permutation_Flag, PriorParameters_Flag_ForPermutation, Rand_Index_ForPermutation):
    ...
    if Permutation_Flag:
        ...
        # Fold12
        if PriorParameters_Flag_ForPermutation:
            Training_Index_Random = Rand_Index_ForPermutation;
            Optimal_Alpha = AlphaRange_Or_PriorAlpha
            Optimal_L1_ratio = L1_ratio_Range_Or_PriorL1ratio
        else:
            ...
            Optimal_Alpha, Optimal_L1_ratio = ElasticNet_OptimalAlpha_KFold(Training_Data, Training_Score, Nested_Fold_Quantity, AlphaRange_Or_PriorAlpha, L1_ratio_Range_Or_PriorL1ratio, ResultantFolder, Parallel_Quantity)
