...
def ElasticNet_APredictB_Permutation(Training_Data, Training_Score, Testing_Data, Testing_Score, Times_IDRange, \
                NestedCV_Flag, AlphaRange_Or_PriorAlpha, L1_ratio_Range_Or_PriorL1ratio, ResultantFolder, Parallel_Quantity, Max_Queued, QueueOptions):
    if not os.path.exists(ResultantFolder):
        os.mkdir(ResultantFolder)
    ...
    for i in np.arange(len(Times_IDRange)):
        ...
        if not os.path.exists(ResultantFolder_I + '/APredictB.mat'):
            ...
            system_cmd = 'python3 -c ' + '\'import sys;\
                ...
                ResultantFolder_I = configuration["ResultantFolder_I"];\
                Parallel_Quantity = configuration["Parallel_Quantity"];\
                ElasticNet_APredictB_Permutation_Sub(Subjects_Data_Mat_Path[0], Subjects_Score_Mat_Path[0], Alpha_Range[0], L1_ratio_Range[0], ResultantFolder_I[0], Parallel_Quantity[0][0])\' ';
            system_cmd = system_cmd + ' > "' + ResultantFolder_I + '/perm_' + str(Times_IDRange[i]) + '.log" 2>&1\n'
...
def ElasticNet_APredictB_Permutation_Sub(Subjects_Data_Mat_Path, Subjects_Score_Mat_Path, NestedCV_Flag, AlphaRange_Or_PriorAlpha, L1_ratio_Range_Or_PriorL1ratio, ResultantFolder, Parallel_Quantity, Rand_Index):
    # Data
    data = sio.loadmat(Subjects_Data_Mat_Path)
    ...
    ElasticNet_APredictB(Training_Data, Training_Score, Testing_Data, Testing_Score, NestedCV_Flag, AlphaRange_Or_PriorAlpha, L1_ratio_Range_Or_PriorL1ratio, 3, ResultantFolder, Parallel_Quantity, 1, Rand_Index)
                            
def ElasticNet_APredictB(Training_Data, Training_Score, Testing_Data, Testing_Score, NestedCV_Flag, AlphaRange_Or_PriorAlpha, L1_ratio_Range_Or_PriorL1ratio, Nested_Fold_Quantity, ResultantFolder, Parallel_Quantity, Permutation_Flag, Rand_Index):
    ...
    if Permutation_Flag:
        ...
        # Fold12
        if Rand_Index:
            Training_Index_Random = Rand_Index;
        else:
