...
def LinearRegression_KFold_Permutation(Subjects_Data, Subjects_Score, Times_IDRange, Fold_Quantity, ResultantFolder):
    ...
    for i in Times_IDRange:
        ...
        LinearRegression_KFold(Subjects_Data, Subjects_Score_Random, Fold_Quantity, ResultantFolder_I)

def LinearRegression_KFold_RandSplit_MTimes_AllSubsets(Subjects_Data_Mat_Path, Subjects_Score, SampleSize_Array, Times_SampleResample, Times_NFold, Fold_Quantity, ResultantFolder, Max_Queued, QueueOptions):
        ...
        for j in np.arange(Times_SampleResample):
            if not os.path.exists(ResultantFolder_I + '/Res_' + str(j) + '.mat'):
                ...
                system_cmd = 'python3 -c ' + '\'import sys;\
                    ...
                    from LeastSquares_CZ import LinearRegression_KFold_RandSplit_MTimes_OneSubset;\
                    import os;\
                    import scipy.io as sio;\
                    ...
                    LinearRegression_KFold_RandSplit_MTimes_OneSubset(Subjects_Data_Mat_Path[0], Subjects_Score[0], SampleSize[0][0], Fold_Quantity[0][0], Times_NFold[0][0], Sample_Index[0][0], ResultantFolder_I[0])\' ';
...
def LinearRegression_KFold_RandSplit_MTimes_OneSubset(Subjects_Data_Mat_Path, Subjects_Score, SampleSize, Fold_Quantity, Times, SampleIndex, ResultantFolder):
    ...
    Mean_Corr_MTimes, Mean_MAE_MTimes, Corr_MTimes, MAE_MTimes = LinearRegression_KFold_RandSplit_MTimes(Data_Selected, Scores_Selected, Fold_Quantity, Times)
...
def LinearRegression_KFold_RandSplit_MTimes(Subjects_Data, Subjects_Score, Fold_Quantity, Times):

    Corr_MTimes = np.zeros(Times);
    MAE_MTimes = np.zeros(Times);
    for i in np.arange(Times):
        Corr_I, MAE_I = LinearRegression_KFold_RandSplit(Subjects_Data, Subjects_Score, Fold_Quantity)
...
def LinearRegression_KFold_RandSplit(Subjects_Data, Subjects_Score, Fold_Quantity):
