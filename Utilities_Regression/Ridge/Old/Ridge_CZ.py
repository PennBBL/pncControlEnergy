...
def Ridge_KFold_Permutation(Subjects_Data, Subjects_Score, Times_IDRange, Fold_Quantity, ResultantFolder):
    ...
    for i in Times_IDRange:
        ...
        Ridge_KFold(Subjects_Data, Subjects_Score_Random, Fold_Quantity, ResultantFolder_I)

def Ridge_KFold_RandSplit_MTimes_AllSubsets(Subjects_Data_Mat_Path, Subjects_Score, SampleSize_Array, Times_SampleResample, Times_NFold, Fold_Quantity, ResultantFolder, Max_Queued, QueueOptions):
        ...
        for j in np.arange(Times_SampleResample):
            if not os.path.exists(ResultantFolder_I + '/Res_' + str(j) + '.mat'):
                ...
                system_cmd = 'python3 -c ' + '\'import sys;\
                    ...
                    from LeastSquares_CZ import Ridge_KFold_RandSplit_MTimes_OneSubset;\
                    import os;\
                    import scipy.io as sio;\
                    ...
                    Ridge_KFold_RandSplit_MTimes_OneSubset(Subjects_Data_Mat_Path[0], Subjects_Score[0], SampleSize[0][0], Fold_Quantity[0][0], Times_NFold[0][0], Sample_Index[0][0], ResultantFolder_I[0])\' ';
...
def Ridge_KFold_RandSplit_MTimes_OneSubset(Subjects_Data_Mat_Path, Subjects_Score, SampleSize, Fold_Quantity, Times, SampleIndex, ResultantFolder):
    ...
    Mean_Corr_MTimes, Mean_MAE_MTimes, Corr_MTimes, MAE_MTimes = Ridge_KFold_RandSplit_MTimes(Data_Selected, Scores_Selected, Fold_Quantity, Times)
...
def Ridge_KFold_RandSplit_MTimes(Subjects_Data, Subjects_Score, Fold_Quantity, Times):

    Corr_MTimes = np.zeros(Times);
    MAE_MTimes = np.zeros(Times);
    for i in np.arange(Times):
        Corr_I, MAE_I = Ridge_KFold_RandSplit(Subjects_Data, Subjects_Score, Fold_Quantity, 1)
...
def Ridge_KFold_RandSplit(Subjects_Data, Subjects_Score, Fold_Quantity, InnerCV_Flag, Alpha_Value):
    ...
    for j in np.arange(Fold_Quantity):
        ...
        if InnerCV_Flag:
            ...
            Optimal_Alpha, Inner_Corr, Inner_MAE_inv = Ridge_OptimalAlpha_KFold(Subjects_Data_train, Subjects_Score_train, Fold_Quantity, Alpha_Range)
            #######################################################################  
        else:
        ...
        clf = linear_model.Ridge(alpha = OptimalAlpha) # Select the optimal alpha
...
def Ridge_OptimalAlpha_KFold(Subjects_Data_train, Subjects_Score_train, Fold_Quantity, Alpha_Range)
    ...
    for i in np.arange(Alpha_Quantity):
        for j in np.arange(5):
            Corr, MAE = Ridge_KFold_RandSplit(Subjects_Data, Subjects_Score, Fold_Quantity, 0, Alpha_Range[i])
            Inner_Corr[j, i] = Corr
...
def Ridge_KFold(Subjects_Data, Subjects_Score, Fold_Quantity, ResultantFolder):
...
def Ridge_APredictB_Permutation(Training_Data, Training_Score, Testing_Data, Testing_Score, Times_IDRange, ResultantFolder):
    ...
    for i in Times_IDRange:
        ...
        if not os.path.exists(ResultantFolder_I):
            os.mkdir(ResultantFolder_I)
        Ridge_APredictB(Training_Data, Training_Score_Random, Testing_Data, Testing_Score, ResultantFolder_I)
    
def Ridge_APredictB(Training_Data, Training_Score, Testing_Data, Testing_Score, ResultantFolder):
...
def Ridge_Weight(Subjects_Data, Subjects_Score, ResultantFolder):
