...
def Ridge_KFold_Permutation(Subjects_Data, Subjects_Score, Times_IDRange, Fold_Quantity, InnerCV_Flag, ResultantFolder, Parallel_Quantity, Max_Queued, Queue):
    ...
    for i in np.arange(len(Times_IDRange)):
        ...
        if not os.path.exists(ResultantFolder_I + '/Res_NFold.mat'):
            ...
            system_cmd = 'python3 -c ' + '\'import sys;\
                sys.path.append("/lustre/shulab/cuizaixu/Utilities_Zaixu/Utilities_Regression/Ridge");\
                from Ridge_CZ import Ridge_KFold_Permutation_Sub;\
                import os;\
                import scipy.io as sio;\
                ...
                Ridge_KFold_Permutation_Sub(Subjects_Data_Mat_Path[0], Subjects_Score[0], l1_ratio[0][0], Fold_Quantity[0][0], InnerCV_Flag[0][0], ResultantFolder_I[0], Parallel_Quantity[0][0])\' ';
...
def Ridge_KFold_Permutation_Sub(Subjects_Data_Mat_Path, Subjects_Score, Fold_Quantity, InnerCV_Flag, ResultantFolder, Parallel_Quantity):
    data = sio.loadmat(Subjects_Data_Mat_Path)
    Subjects_Data = data['Subjects_Data']
    Ridge_KFold(Subjects_Data, Subjects_Score, Fold_Quantity, InnerCV_Flag, ResultantFolder, Parallel_Quantity, 1);
   
def Ridge_KFold(Subjects_Data, Subjects_Score, Fold_Quantity, InnerCV_Flag, ResultantFolder, Parallel_Quantity, Permutation_Flag):
    ...
    for j in np.arange(Fold_Quantity):
        ...
        if InnerCV_Flag:
            ...
            Optimal_Alpha, Inner_Corr, Inner_MAE_inv = Ridge_OptimalAlpha_KFold(Subjects_Data_train, Subjects_Score_train, Fold_Quantity, l1_ratio, Alpha_Range, ResultantFolder, Parallel_Quantity)
            #######################################################################  
        else:
            Optimal_Alpha = 1
...
def Ridge_Weight(Subjects_Data, Subjects_Score, CV_Flag, CVFoldQuantity_Or_Alpha, ResultantFolder):
...
# if CV_Flag is 0, CVFoldQuantity_Or_Alpha is alpha
    ...
    if CV_Flag:
        ...
        Optimal_Alpha, Inner_Corr, Inner_MAE_inv = Ridge_OptimalAlpha_KFold(Subjects_Data, Subjects_Score, CVFoldQuantity_Or_Alpha, Alpha_Range, ResultantFolder, Parallel_Quantity)
    else:
...
def Ridge_Weight_BootStrap(Subjects_Data, Subjects_Score, CV_Flag, CVFoldQuantity_Or_Alpha, Times_Range, ResultantFolder):
# if CV_Flag is 1, CVFoldQuantity_Or_Alpha is the quantity of folds
# if CV_Flag is 0, CVFoldQuantity_Or_Alpha is alpha
    ...
    for i in Times_Range:
        ...
        if CV_Flag:
            # Select optimal alpha using inner fold cross validation
            Parallel_Quantity = 30
            Optimal_Alpha, Inner_Corr, Inner_MAE_inv = Ridge_OptimalAlpha_KFold(Subjects_Data_Selected, Subjects_Score_Selected, CVFoldQuantity_Or_Alpha, Alpha_Range, ResultantFolder, Parallel_Quantity)
        else:
            Optimal_Alpha = CVFoldQuantity_Or_Alpha;
...
def Ridge_Weight_L2(Subjects_Data, Subjects_Score, CV_Flag, CVFoldQuantity_Or_Alpha, ResultantFolder):
...
def Ridge_Weight_L2_Permutation(Subjects_Data, Subjects_Score, Times_IDRange, Alpha, ResultantFolder):
    ...
    for i in np.arange(RangeQuantity):
        ...
        Parallel(n_jobs=Parallel_Quantity)(delayed(Ridge_Weight_L2)(Subjects_Data, Subjects_Score_Random[:, l], Alpha, ResultantFolder_I[l]) for l in np.arange(Jobs_Quantity))      
