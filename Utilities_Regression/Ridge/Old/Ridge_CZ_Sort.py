...
def Ridge_KFold_Sort_AllSubsets(Subjects_Data_Mat_Path, Subjects_Score, SampleSize_Array, Times_SampleResample, Fold_Quantity, Alpha_Range, ResultantFolder, Max_Queued, QueueOptions, Parallel_Quantity):
        ...
        for j in np.arange(Times_SampleResample):
            if not os.path.exists(ResultantFolder_I + '/Prediction_' + str(j) + '.mat'):
                ...
                Configuration_Mat = {'Subjects_Data_Mat_Path': Subjects_Data_Mat_Path, 'Subjects_Score': Subjects_Score, 'SampleSize': SampleSize_Array[i], \
                    'Fold_Quantity': Fold_Quantity, 'Sample_Index': j, 'Alpha_Range': Alpha_Range, 'ResultantFolder_I': ResultantFolder_I, 'Parallel_Quantity': Parallel_Quantity};
                ...
                system_cmd = 'python3 -c ' + '\'import sys;\
                    ...
                    Ridge_KFold_Sort_OneSubset(Subjects_Data_Mat_Path[0], Subjects_Score[0], SampleSize[0][0], Sample_Index[0][0], Fold_Quantity[0][0], Alpha_Range[0], ResultantFolder_I[0], Parallel_Quantity[0][0])\' ';
...
def Ridge_KFold_Sort_OneSubset(Subjects_Data_Mat_Path, Subjects_Score, SampleSize, SampleIndex, Fold_Quantity, Alpha_Range, ResultantFolder, Parallel_Quantity):
...
def Ridge_KFold_Sort(Subjects_Data, Subjects_Score, Fold_Quantity, Alpha_Range, ResultantFolder, Parallel_Quantity):
