...
Energy_Mat_Path <- paste(Energy_Data_Folder, '/SC_Energy/SC_InitialDM1_TargetActivation_ScaleFactor_3000.mat', sep = '/');
...
ResultantFolder <- '/data/joy/BBL/projects/pncControlEnergy/results/InitialDM1_TargetActivation/Development_Effects/NodalLevel';
...
ResultantFolder <- '/data/joy/BBL/projects/pncControlEnergy/results/InitialDM1_TargetActivation/Development_Effects/YeoSystemLevel';
...
# Display interaction of subcortical
tmp_variable = Energy_YeoAvg[, 8];
Energy_Gam <- gam(tmp_variable ~ s(Age_years, by = Sex_order, k = 4) + s(Age_years, k = 4) + Sex_order + HandednessV2 + MotionMeanRelRMS + TBV, method = "REML", data = Behavior);
visreg(Energy_Gam, "Age_years", by = "Sex_order", overlay = TRUE, partial = FALSE, xlab = "Age (years)", ylab = "Average energy of subcortical");
# Display interaction of sum of FP and DM
tmp_variable = Energy_YeoAvg[, 6] + Energy_YeoAvg[, 7];
Energy_Gam <- gam(tmp_variable ~ s(Age_years, by = Sex_order, k = 4) + s(Age_years, k = 4) + Sex_order + HandednessV2 + MotionMeanRelRMS + TBV, method = "REML", data = Behavior);
visreg(Energy_Gam, "Age_years", by = "Sex_order", overlay = TRUE, partial = FALSE, xlab = "Age (years)", ylab = "Sum of FP and DM energy");
