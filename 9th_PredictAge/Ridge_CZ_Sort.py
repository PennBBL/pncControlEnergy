    ...
    for i in np.arange(len(Times_IDRange)):
        ...
        if not os.path.exists(ResultantFolder_I + '/Res_NFold.mat'):
            ...
            system_cmd = 'python3 -c ' + '\'import sys;\
                sys.path.append("/data/jux/BBL/projects/pncControlEnergy/scripts/Replication_20180616_DivededByActivation/8th_PredictAge");\
    ...
    for i in np.arange(len(Times_IDRange)):
        ...
        if not os.path.exists(ResultantFolder_I + '/APredictB.mat'):
            ...
            system_cmd = 'python3 -c ' + '\'import sys;\
                ...
                from Ridge_CZ_Sort import Ridge_APredictB_Permutation_Sub;\
                import os;\
                import scipy.io as sio;\
                configuration = sio.loadmat("'+ ResultantFolder_I + '/Configuration.mat");\
                Subjects_Data_Mat_Path = configuration["Subjects_Data_Mat_Path"];\
...
def Ridge_APredictB(Training_Data, Training_Score, Testing_Data, Testing_Score, CV_Flag, CV_FoldQuantity_or_Alpha, Alpha_Range, ResultantFolder, Parallel_Quantity):
    ...
    Weight = clf.coef_ / np.sqrt(np.sum(clf.coef_ **2)) # Save weight
    ...
    if CV_Flag:
        Predict_result = {'Test_Score':Testing_Score, 'Predict_Score':Predict_Score, 'Weight':clf.coef_, 'Predict_Corr':Predict_Corr, 'Predict_MAE':Predict_MAE, 'alpha':Optimal_Alpha, 'Inner_Corr':Inner_Corr, 'Inner_MAE_inv':Inner_MAE_inv, 'w_Brain':Weight}
    else:
        Predict_result = {'Test_Score':Testing_Score, 'Predict_Score':Predict_Score, 'Weight':clf.coef_, 'Predict_Corr':Predict_Corr, 'Predict_MAE':Predict_MAE, 'w_Brain':Weight}
