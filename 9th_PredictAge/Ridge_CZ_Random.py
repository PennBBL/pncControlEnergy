...
def Ridge_KFold_RandomCV_MultiTimes_Permutation(Subjects_Data, Subjects_Score, Times_IDRange, Fold_Quantity, Alpha_Range, CVRepeatTimes, ResultantFolder, Parallel_Quantity, Max_Queued, Queue):
    ...
    for i in np.arange(len(Times_IDRange)):
        ...
        if not os.path.exists(ResultantFolder_I + '/Res_NFold.mat'):
            ...
            Configuration_Mat = {'Subjects_Data_Mat_Path': Subjects_Data_Mat_Path, 'Subjects_Score': Subjects_Score, 'Fold_Quantity': Fold_Quantity, \
                'Alpha_Range': Alpha_Range, 'CVRepeatTimes':CVRepeatTimes, 'ResultantFolder_I': ResultantFolder_I, 'Parallel_Quantity': Parallel_Quantity};
            sio.savemat(ResultantFolder_I + '/Configuration.mat', Configuration_Mat)
            system_cmd = 'python3 -c ' + '\'import sys;\
                sys.path.append("/data/jux/BBL/projects/pncControlEnergy/scripts/Replication_20180611/8th_PredictAge");\
                from Ridge_CZ_Random import Ridge_KFold_RandomCV_MultiTimes_Permutation_Sub;\
                import os;\
                import scipy.io as sio;\
                ...
                Ridge_KFold_RandomCV_MultiTimes_Permutation_Sub(Subjects_Data_Mat_Path[0], Subjects_Score[0], Fold_Quantity[0][0], Alpha_Range[0], CVRepeatTimes[0], ResultantFolder_I[0], Parallel_Quantity[0][0])\' ';
...
def Ridge_KFold_RandomCV_MultiTimes_Permutation_Sub(Subjects_Data_Mat_Path, Subjects_Score, Fold_Quantity, Alpha_Range, CVRepeatTimes, ResultantFolder, Parallel_Quantity):
    data = sio.loadmat(Subjects_Data_Mat_Path)
    Subjects_Data = data['Subjects_Data']
    Ridge_KFold_RandomCV_MultiTimes(Subjects_Data, Subjects_Score, Fold_Quantity, Alpha_Range, CVRepeatTimes, ResultantFolder, Parallel_Quantity, 1);
