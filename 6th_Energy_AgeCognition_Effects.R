...
{
  RowName_Nodal[i] = paste("Node", as.character(SmallerThanNull_Index[i]));
}
ColName <- c("Z", "P", "P_FDR");
...
Energy_Gam_Age_New <- matrix(100, nrow = 233, ncol = 3, dimnames = list(RowName_Nodal, ColName));
...
Energy_Gam_Cognition <- matrix(c(1:RegionsQuantity*2), nrow = RegionsQuantity, ncol = 2, dimnames = list(RowName_Nodal, ColName));
for (i in 1:RegionsQuantity)
{
...
Energy_Gam_Cognition_New <- matrix(100, nrow = 233, ncol = 2, dimnames = list(RowName_Nodal, ColName));
